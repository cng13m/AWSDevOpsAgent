...
resource "aws_s3_bucket" "trail" {
  bucket = "${local.prefix}-trail"
  tags   = merge(var.common_tags, { Name = "${var.project_name}-${var.environment}-trail" })
}

resource "aws_s3_bucket" "config" {
  bucket = "${local.prefix}-config"
  tags   = merge(var.common_tags, { Name = "${var.project_name}-${var.environment}-config" })
}

resource "aws_s3_bucket" "alb_logs" {
  bucket = "${local.prefix}-alb-logs"
  tags   = merge(var.common_tags, { Name = "${var.project_name}-${var.environment}-alb-logs" })
}
